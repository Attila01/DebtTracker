...
# Version: 1.5 (2025-07-16) - Enhanced error handling for CreateExcelTemplate.ps1 execution.
...
try {
    ...
    Write-SyncLog "Running database initialization script: $dbInitScriptPath"
    ...
    Write-SyncLog 'Database initialization completed.'

    # --- Step 2: Robustly close any running Excel processes and COM objects ---
    Write-SyncLog 'Attempting robust closure of Excel processes and COM objects...'
    ...
    Write-SyncLog 'Excel process and COM object termination attempt completed.'

    # --- Step 3: Create/Update Excel Template ---
    Write-SyncLog "Running Excel template creation script: $excelTemplateScriptPath"
    ...
    try {
        ...
        Write-SyncLog 'Excel template creation completed.'
    }
    catch {
        $actualErrorMessage = $_.Exception?.Message ?? $_.ToString()
        Write-SyncLog "Error during Excel template creation script execution: ${actualErrorMessage}"
    ...
    Write-SyncLog 'Attempting robust closure of Excel processes and COM objects before UI launch...'
    Get-Process -Name 'EXCEL' -ErrorAction SilentlyContinue | ForEach-Object {
        Write-SyncLog "Stopping Excel process with ID $($_.Id)."
        Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue
    }
    ...
    [System.GC]::WaitForPendingFinalizers()
    Start-Sleep -Seconds 2 # Give more time for processes/objects to release
    Write-SyncLog 'Second Excel process and COM object termination attempt completed.'

    # --- Step 5: Launch Main UI Script ---
    Write-SyncLog "Launching DebtManagerUI.ps1: $uiScriptPath"
    ...
    Write-SyncLog 'DebtManagerUI.ps1 launched successfully.'

}
catch {
    $actualErrorMessage = $_.Exception?.Message ?? $_.ToString()
    Write-SyncLog "Startup error in SyncAndLaunch.ps1: ${actualErrorMessage}"
    [System.Windows.Forms.MessageBox]::Show("Startup error: ${actualErrorMessage}", 'Error', 'OK', 'Error')
}
